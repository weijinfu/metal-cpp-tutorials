...
这些提示词用于把当前 SVG 图解重绘成更有教学质感的 PNG。建议所有图保持同一视觉语言：干净、平面、浅色纸张背景、少量强调色、标签清晰，不使用复杂透视。
...
Use case: scientific-educational
Asset type: diagram for a programming tutorial web book
Primary request: Compare Metal compute pipeline and render pipeline for beginners.
